...
type Stock struct {
	...
	CompanyNameVi string    `json:"company_name_vi"`
...
func StockBobToRaw(item *bob.Stock) *Stock {
	if item == nil {
		return nil
	}
	...
	return &Stock{
		Symbol:        item.Symbol,
		CompanyNameVi: item.CompanyNameVi.GetOrZero(),
		CompanyNameEn: item.CompanyNameEn.GetOrZero(),
		Exchange:      item.Exchange.GetOrZero(),
		Isin:          item.Isin.GetOrZero(),
...
type StockTrackingInfo struct {
	...
	ClientNameEn               string  `json:"clientNameEn"`
}
